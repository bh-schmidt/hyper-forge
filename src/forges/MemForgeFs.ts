import { IForgeFs, TempPathType, WriteOptions } from '@/forges/Types';
...
export class MemForgeFs implements IForgeFs {
    ...
    async injectDirectory(path: string, variables?: any) {
        ...
        const newPath = await this.forge.nameInjector.inject(undefined, path, variables)
    ...
    async* getDirs(sort: boolean = false) {
        ...
        for await (const item of directories) {
            const path = Utils.Path.isDirectory(item.targetPath) ?
                item.targetPath :
                item.targetPath + sep
