import { FileExistsAction, TempPathType, WriteOptions } from '@/forges/Types';
...
export class TempFs {
    ...
    constructor(private tempDir: string, private fileInjector: FileInjector, private nameInjector: NameInjector) {
    ...
    * getTempPaths(filters?: Partial<TempPath>, sort: boolean = false) {
    ...
    async injectFile(src: string, dest: string, variables: any, options: WriteOptions = {}) {
        ...
        const newPath = await this.nameInjector.inject(src, dest, variables)
        ...
        if (await this.fileInjector.shouldInject(src, temp.tempPath!, variables)) {
            try {
                await this.fileInjector.inject(src, temp.tempPath!, variables)
                return
            } catch (error) {
    ...
    private async clearFiles() {
        const tempFiles = Globber.iterator(
            '**/*.temp',
            {
                cwd: this.tempDir,
                nodir: true,
